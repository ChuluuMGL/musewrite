...
async function main() {
  switch (command) {
    ...
    case 'knowledge': await runKnowledge(args.slice(1)); break;
    case 'brand': await runKnowledge(['brand', ...args.slice(1)]); break;
    case 'product': await runKnowledge(['product', ...args.slice(1)]); break;
    case 'project': await runKnowledge(['project', ...args.slice(1)]); break;
    case 'interview':
      // 委托给子命令
...
async function runDraft(args) {
  ...
  if (cmd === 'list' || cmd === 'ls') {
    ...
    if (diff.differences.contentLength.changed) {
      console.log(`  内容长度: ${diff.differences.contentLength.old} → ${diff.differences.contentLength.new}`);
    }
...
async function runKnowledge(args) {
  const cmd = args[0];
  const KnowledgeManager = require(path.join(ROOT, 'lib/KnowledgeManager'));
  const DATA_PATH = path.join(ROOT, 'data');
  const km = new KnowledgeManager(DATA_PATH);

  if (cmd === 'project' || cmd === 'pj') {
    const subCmd = args[1];
    if (subCmd === 'create') {
      const name = args[2];
      if (!name) return console.log('❌ 请指定项目名称');
      const desc = args.indexOf('-d') > -1 ? args[args.indexOf('-d') + 1] : '';
      const project = km.createProject({ name, description: desc });
      console.log(`✅ 项目已创建`);
      console.log(`   ID: ${project.id}`);
      console.log(`   名称: ${project.name}`);
    } else if (subCmd === 'list' || subCmd === 'ls') {
      const projects = km.listProjects();
      if (!projects.length) return console.log('📭 暂无项目');
      console.log(`📋 项目列表 (${projects.length} 个):\n`);
      projects.forEach((p, i) => {
        console.log(`${i+1}. ${p.name} (${p.brandCount} 品牌, ${p.accountCount} 账号)`);
        console.log(`   ID: ${p.id}`);
      });
    } else {
      console.log(`用法:
  project create <name> [-d "描述"]    创建项目
  project list                        列出项目`);
    }
  } else if (cmd === 'brand') {
    const subCmd = args[1];
    if (subCmd === 'create') {
      const name = args[2];
      const projectId = args.indexOf('-p') > -1 ? args[args.indexOf('-p') + 1] : null;
      const tone = args.indexOf('-t') > -1 ? args[args.indexOf('-t') + 1]?.split(',') : [];
      const positioning = args.indexOf('-pos') > -1 ? args[args.indexOf('-pos') + 1] : '';
      if (!name) return console.log('❌ 请指定品牌名称');
      const brand = km.createBrand({ name, projectId, tone, positioning });
      console.log(`✅ 品牌已创建: ${brand.id}`);
    } else if (subCmd === 'list' || subCmd === 'ls') {
      const projectId = args.indexOf('-p') > -1 ? args[args.indexOf('-p') + 1] : null;
      const brands = km.listBrands(projectId);
      if (!brands.length) return console.log('📭 暂无品牌');
      console.log(`🏢 品牌列表 (${brands.length} 个):\n`);
      brands.forEach((b, i) => {
        console.log(`${i+1}. ${b.name} (${b.industry || '未知行业'})`);
        if (b.positioning) console.log(`   定位: ${b.positioning}`);
        if (b.tone?.length) console.log(`   调性: ${b.tone.join('、')}`);
      });
    } else if (subCmd === 'show') {
      const brandId = args[2];
      if (!brandId) return console.log('❌ 请指定品牌 ID');
      const brand = km.getBrand(brandId);
      if (!brand) return console.log('❌ 品牌不存在');
      console.log(`\n🏢 ${brand.fullName || brand.name}`);
      console.log(`   行业: ${brand.industry || '-'}`);
      console.log(`   定位: ${brand.positioning || '-'}`);
      console.log(`   调性: ${brand.tone?.join('、') || '-'}`);
      console.log(`   关键词: ${brand.keywords?.join('、') || '-'}`);
      if (brand.taboos?.length) {
        console.log(`   ⚠️ 禁忌: ${brand.taboos.join('、')}`);
      }
      // 显示产品
      const products = km.listProducts(brandId);
      console.log(`\n   📦 产品 (${products.length} 个):`);
      products.forEach(p => console.log(`      - ${p.name}`));
    } else {
      console.log(`用法:
  brand create <name> [-p projectId] [-t "调性1,调性2"]    创建品牌
  brand list [-p projectId]                                列出品牌
  brand show <brandId>                                     查看品牌详情`);
    }
  } else if (cmd === 'product') {
    const subCmd = args[1];
    if (subCmd === 'create') {
      const name = args[2];
      const brandId = args.indexOf('-b') > -1 ? args[args.indexOf('-b') + 1] : null;
      const points = args.indexOf('-s') > -1 ? args[args.indexOf('-s') + 1]?.split('|') : [];
      const scenarios = args.indexOf('-sc') > -1 ? args[args.indexOf('-sc') + 1]?.split(',') : [];
      if (!name) return console.log('❌ 请指定产品名称');
      const product = km.createProduct({ name, brandId, sellingPoints: points, scenarios });
      console.log(`✅ 产品已创建: ${product.id}`);
    } else if (subCmd === 'list' || subCmd === 'ls') {
      const brandId = args.indexOf('-b') > -1 ? args[args.indexOf('-b') + 1] : null;
      const products = km.listProducts(brandId);
      if (!products.length) return console.log('📭 暂无产品');
      console.log(`📦 产品列表 (${products.length} 个):\n`);
      products.forEach((p, i) => {
        console.log(`${i+1}. ${p.name} (${p.category || '未分类'})`);
        if (p.sellingPoints?.length) {
          console.log(`   卖点: ${p.sellingPoints.slice(0, 2).join('; ')}`);
        }
      });
    } else {
      console.log(`用法:
  product create <name> [-b brandId] [-s "卖点1|卖点2"]    创建产品
  product list [-b brandId]                                列出产品`);
    }
  } else if (cmd === 'style') {
    const subCmd = args[1];
    if (subCmd === 'show') {
      const accountId = args[2];
      if (!accountId) return console.log('❌ 请指定账号 ID');
      const style = km.getStyle(accountId);
      if (!style) return console.log('📭 暂无风格数据');
      console.log(`✍️ ${accountId} 的写作风格:\n`);
      const s = style.learnedStyle;
      if (s.sentenceLength) console.log(`   句长: ${s.sentenceLength}`);
      if (s.keywords?.length) console.log(`   常用词: ${s.keywords.join('、')}`);
      if (s.emojiUsage) console.log(`   表情: ${s.emojiUsage}`);
      if (s.hashtagStyle) console.log(`   标签: ${s.hashtagStyle}`);
    } else {
      console.log(`用法:
  style show <accountId>    查看账号风格`);
    }
  } else if (cmd === 'memory') {
    const subCmd = args[1];
    if (subCmd === 'add') {
      const projectId = args[2];
      const accountId = args.indexOf('-a') > -1 ? args[args.indexOf('-a') + 1] : null;
      const type = args.indexOf('-t') > -1 ? args[args.indexOf('-t') + 1] : 'success';
      const content = args.indexOf('-c') > -1 ? args[args.indexOf('-c') + 1] : '';
      const reason = args.indexOf('-r') > -1 ? args[args.indexOf('-r') + 1] : '';
      if (!projectId) return console.log('❌ 请指定项目 ID');
      const memory = km.addMemory(projectId, accountId, { type, content, reason });
      console.log(`✅ 记忆已添加`);
    } else if (subCmd === 'insights') {
      const projectId = args[2];
      const accountId = args.indexOf('-a') > -1 ? args[args.indexOf('-a') + 1] : null;
      if (!projectId) return console.log('❌ 请指定项目 ID');
      const insights = km.getInsights(projectId, accountId);
      console.log(`💡 创作洞察:\n`);
      if (insights.taboos?.length) {
        console.log(`⚠️ 禁忌:`);
        insights.taboos.forEach(t => console.log(`   - ${t.content} (${t.reason})`));
      }
      if (insights.topPerformers?.length) {
        console.log(`\n🏆 表现最佳:`);
        insights.topPerformers.forEach(p => console.log(`   - ${p.title} (${p.likes} 赞)`));
      }
    } else {
      console.log(`用法:
  memory add <projectId> [-a accountId] [-t type] [-c "内容"]    添加记忆
  memory insights <projectId> [-a accountId]                    查看洞察`);
    }
  } else {
    console.log(`📚 MuseWrite 知识系统

用法:
  musewrite project <command>     项目管理
  musewrite brand <command>       品牌管理
  musewrite product <command>     产品管理
  musewrite style <command>       风格管理
  musewrite memory <command>      创作记忆

示例:
  # 创建项目
  musewrite project create "蒙牛内容运营"

  # 创建品牌
  musewrite brand create "蒙牛" -p <projectId> -t "温暖,健康,品质"

  # 创建产品
  musewrite product create "特仑苏" -b <brandId> -s "专属牧场|3.6g乳蛋白"

  # 查看创作洞察
  musewrite memory insights <projectId>
`);
  }
}
