  ...
  try {
    ...
    if (type === 'generate') {
      ...
      if (params.checkFeedback) {
        checklist = feedbackManager.getChecklist(params.platform || 'xiaohongshu', params.info || 'stone');
      }
  ...
  while (true) {
    try {
      ...
      for (const taskInfo of pendingTasks) {
        ...
        // 异步处理任务
        processTask(taskQueue.tasks[taskInfo.id])
          .finally(() => runningTasks.delete(taskInfo.id));
      }
