...
async function handleGenerate(body, taskId = null) {
  ...
  if (taskId) {
    ...
    taskQueue.updateProgress(taskId, 10, 'Starting...');
  }
  ...
  return {
    ...
    image: draft.image ? {filename: draft.image.filename, path: draft.image.path} : null,
    filename
  };
}
  ...
  try {
    ...
    if (pathname === '/' && req.method === 'GET') {
      ...
      res.end(
        JSON.stringify({
          success: true,
          service: 'MuseWrite API',
          version: '1.0.0',
          port,
          timestamp: new Date().toISOString(),
          features: {authentication: true, rateLimiting: true, idempotency: true, openapi: true},
          tasks: {
            total: Object.keys(taskQueue.tasks).length,
            pending: Object.values(taskQueue.tasks).filter((t) => t.status === 'pending').length,
            running: Object.values(taskQueue.tasks).filter((t) => t.status === 'running').length
          },
          apiKeys: {
            total: auth.keys.keys.length,
            active: auth.keys.keys.filter((k) => k.lastUsed).length
          }
        })
      );
      ...
      if (!req.user || !req.user.permissions.includes('admin')) {
        res.writeHead(403);
        res.end(JSON.stringify({success: false, error: 'Admin permission required'}));
        return;
      }
      const body = await readBody(req);
      const keyInfo = auth.generateKey(
        body.name || 'unnamed',
      ...
      const body = await readBody(req);
      const record = publishTracker.recordPublish(
        body.draftId,
        body.platform,
        body.url,
        body.metadata
      ...
      const drafts = files.map((f) => {
        const d = JSON.parse(fs.readFileSync(path.join(DRAFTS_PATH, f)));
        return {
          filename: f,
          title: d.title,
          platform: d.platform,
    ...
    // ==================== 版本管理 API ====================
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/versions\/\d+$/) &&
      req.method === 'GET'
    ) {
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/rollback\/\d+$/) &&
      req.method === 'POST'
    ) {
      const parts = pathname.split('/');
      const draftId = parts[4];
      ...
      res.writeHead(200);
      res.end(
        JSON.stringify({success: true, draft: restored, message: `已回滚到版本 ${targetVersion}`})
      );
    } else if (
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/compare\/\d+\/\d+$/) &&
      req.method === 'GET'
    ) {
      const parts = pathname.split('/');
      const draftId = parts[4];
    ...
    // ==================== 审核流程 API ====================
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/review\/submit$/) &&
      req.method === 'POST'
    ) {
      const draftId = pathname.split('/')[4];
      const body = await readBody(req);
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/review\/approve$/) &&
      req.method === 'POST'
    ) {
      const draftId = pathname.split('/')[4];
      const body = await readBody(req);
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/review\/reject$/) &&
      req.method === 'POST'
    ) {
      const draftId = pathname.split('/')[4];
      const body = await readBody(req);
      if (!body.reason) {
        res.writeHead(400);
        res.end(JSON.stringify({success: false, error: 'Reason is required'}));
        return;
      }
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/review\/changes$/) &&
      req.method === 'POST'
    ) {
      const draftId = pathname.split('/')[4];
      const body = await readBody(req);
      ...
      pathname.match(/^\/api\/v1\/drafts\/[^/]+\/review\/resubmit$/) &&
      req.method === 'POST'
    ) {
      const draftId = pathname.split('/')[4];
      const body = await readBody(req);
      ...
      if (!review) {
        res.writeHead(404);
        res.end(JSON.stringify({success: false, error: 'No review found'}));
        return;
      }
      res.writeHead(200);
      res.end(JSON.stringify({success: true, review}));
