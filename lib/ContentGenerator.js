...
class ContentGenerator {
   ...
   * 生成内容
   */
  async generate({ source, infoCard, styleCard, platformCard, platform, withImage = false, model, apiKey }) {
    ...
    if (model || apiKey) {
      ...
      const llmConfig = conf.getLlmConfig();
      
      let targetProvider = this.llm.provider;
      let targetModel = model || this.llm.config.model;
      let targetApiKey = apiKey || this.llm.config.apiKey;

      if (model && model !== this.llm.config.model) {
        const matched = llmConfig.providers.find(p => p.model === model);
        if (matched) {
          targetProvider = matched.name;
          if (!apiKey) {
            // 从后端配置回退获取该服务商对应的 API Key
            if (matched.env === 'GOOGLE_API_KEY') targetApiKey = conf.get('GEMINI_API_KEY');
            else if (matched.env) targetApiKey = conf.get(matched.env);
          }
        }
      }

      activeLlm = new LLMProvider({
    ...
    // 调用 LLM
    const rawContent = await activeLlm.chat(prompt, { model: activeLlm.config.model });
