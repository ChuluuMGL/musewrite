  ...
  return {
    'POST /api/v1/chat/test': async (req, res, params, body) => {
      try {
        ...
        if (response) {
          res.writeHead(200);
          res.end(JSON.stringify({ success: true, message: action === 'ping' ? 'pong' : 'success', rawResponse: response }));
        } else {
          res.writeHead(500);
        ...
        res.writeHead(500);
        res.end(JSON.stringify({ success: false, error: error.message || 'Unknown error occurred during chat request' }));
      }
    }
  };
};
