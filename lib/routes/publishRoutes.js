...
module.exports = function (rootPath) {
  ...
  return {
    ...
    'POST /api/v1/publish/batch': async (req, res, params, body) => {
      const {draftId, platforms, options, content} = body;

      // 支持 direct content 或 draftId
      let publishContent = content;

      if (!publishContent && draftId) {
        // 读取草稿内容
        const draftsPath = path.join(rootPath, 'drafts');
        const files = fs.readdirSync(draftsPath).filter((f) => f.endsWith('.json'));
        const draftFile = files.find((f) => f.includes(draftId) || f === `${draftId}.json`);
      ...
      if (!publishContent || !platforms || !Array.isArray(platforms)) {
        res.writeHead(400);
        res.end(
          JSON.stringify({
            success: false,
            error: 'content (or draftId) and platforms array are required'
          })
        );
        return;
      }
      ...
      try {
        const result = await publisher.publishMultiple(platforms, publishContent, options);
    ...
    // 获取支持的平台列表
    'GET /api/v1/publish/platforms': (req, res, _params) => {
