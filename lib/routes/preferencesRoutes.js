  ...
  return {
    ...
    'GET /api/v1/preferences': (req, res, params) => {
      ...
      res.end(JSON.stringify({
        success: true,
        preferences: {
          avoid: preferences.rules.filter(r => r.type === 'avoid' && r.confidence > 0.5).map(r => r.value),
          prefer: preferences.rules.filter(r => r.type === 'prefer' && r.confidence > 0.5).map(r => r.value),
          style: preferences.rules.filter(r => r.type === 'style').map(r => ({ value: r.value, confidence: r.confidence })),
