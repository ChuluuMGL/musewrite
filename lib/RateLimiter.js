...
class RateLimiter {
  ...
  checkLimit(identifier) {
    ...
    if (minuteRequests >= this.requestsPerMinute) {
      return { allowed: false, reason: 'rate_limit_minute', retryAfter: Math.ceil((bucket.requests[minuteRequests - this.requestsPerMinute] + 60000 - now) / 1000) };
    }
    const hourRequests = bucket.requests.filter(t => t > now - 3600000).length;
    if (hourRequests >= this.requestsPerHour) {
      return { allowed: false, reason: 'rate_limit_hour', retryAfter: Math.ceil((bucket.requests[hourRequests - this.requestsPerHour] + 3600000 - now) / 1000) };
    }
  ...
  cleanup() {
    ...
    for (const [identifier, bucket] of this.buckets.entries()) {
      if (bucket.requests.length === 0 || bucket.requests[bucket.requests.length - 1] < now - 3600000) this.buckets.delete(identifier);
    }
  }
  ...
  middleware(req, res) {
    ...
    if (!result.allowed) {
      res.writeHead(429, { 'Content-Type': 'application/json', 'Retry-After': result.retryAfter.toString() });
      res.end(JSON.stringify({ success: false, error: 'Too Many Requests', message: result.reason === 'rate_limit_minute' ? '分钟请求数超限' : '小时请求数超限', retryAfter: result.retryAfter }));
      return false;
    }
    return true;
  }
