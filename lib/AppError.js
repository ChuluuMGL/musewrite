...
AppError.unauthorized = (message = '未授权访问') =>
...
AppError.llmError = (message, details) =>
  new AppError(ErrorCodes.LLM_ERROR, message, details);

AppError.llmTimeout = (timeout) =>
  new AppError(ErrorCodes.LLM_TIMEOUT, `LLM请求超时(${timeout}ms)`);
...
AppError.configError = (message, details) =>
  new AppError(ErrorCodes.CONFIG_ERROR, message, details);
