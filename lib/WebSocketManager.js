...
class WebSocketManager {
  ...
  start(httpServer = null) {
    ...
    this.server.on('connection', (ws, req) => {
      ...
      console.log(`[WS] 客户端连接: ${clientId} (总数: ${this.clients.size})`);
    ...
    this.heartbeatInterval = setInterval(() => {
      this.server.clients.forEach((ws) => {
        ...
        if (!client.isAlive) {
          console.log(`[WS] 客户端超时断开: ${client.id}`);
          ws.terminate();
    ...
    console.log(`[WS] WebSocket服务器已启动，端口: ${this.port}`);
    return this;
  }
  ...
  stop() {
    ...
    if (this.server) {
      this.server.close(() => {
        console.log('[WS] WebSocket服务器已停止');
      });
    }
  ...
  _handleDisconnect(clientId) {
    ...
    console.log(`[WS] 客户端断开: ${clientId} (剩余: ${this.clients.size})`);
  }
  ...
  _subscribe(clientId, room) {
    ...
    console.log(`[WS] 客户端 ${clientId} 订阅房间: ${room}`);
  }
  ...
  notifySystem(level, message, data = {}) {
    const event = level === 'alert'
      ? this.Events.SYSTEM_ALERT
      : this.Events.SYSTEM_NOTIFICATION;
