...
class CreatorTwin {
  ...
  getFingerprintSummary() {
    ...
    if (fp.language.sentenceLength.style !== 'unknown') {
      parts.push(`句子风格：${fp.language.sentenceLength.style === 'short' ? '短句为主，节奏明快' : '长句为主，表达详尽'}`);
    }
...
`;
  ...
  _buildStyleInjectionPrompt(platform = 'default') {
    ...
    if (fp.language.emojiUsage.density > 0.5) {
      lines.push(`\n### Emoji使用`);
      lines.push(`适度使用Emoji增加趣味，常用：${fp.language.emojiUsage.favorites.slice(0, 5).join(' ')}`);
    ...
    if (prefs.rules.length > 0) {
      ...
      if (grouped.avoid.length) {
        lines.push(`避免：${grouped.avoid.slice(0, 5).map(r => r.value).join('、')}`);
      }
      if (grouped.prefer.length) {
        lines.push(`偏好：${grouped.prefer.slice(0, 5).map(r => r.value).join('、')}`);
      }
    }
  ...
  _load() {
    try {
      ...
      if (fs.existsSync(preferencesPath)) {
        this.preferences = { ...this.preferences, ...JSON.parse(fs.readFileSync(preferencesPath, 'utf-8')) };
      }
    } catch (e) {
  ...
  _calculateDistribution(values) {
    ...
    for (let i = 0; i < buckets; i++) {
      ...
      const count = sorted.filter(v => v >= start && (i === buckets - 1 ? v <= end : v < end)).length;
  ...
  _addPreferenceRule(newRule) {
    const existing = this.preferences.rules.find(r =>
      r.type === newRule.type && r.value === newRule.value
    );
