...
class BasePublisher {
  ...
  // 子类必须实现
  async publish(_draft) {
    throw new Error('Must implement publish()');
  }
  async preview(_draft) {
    throw new Error('Must implement preview()');
  }
...
class FeishuPublisher extends BasePublisher {
  ...
  async publish(draft) {
    ...
    const _token = await this._getAccessToken();
    ...
    if (blocks.length > 0) {
      await this._request(
        'POST',
        `/open-apis/docx/v1/documents/${documentId}/blocks/batch_create`,
        {
          children: blocks,
          index: null
        }
