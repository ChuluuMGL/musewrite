...
class ConfigManager {
  ...
  getLlmConfig() {
    ...
    const providers = allProviders.map((p) => {
      const apiKey = p.env
        ? this.get(p.env) || (p.env === "GOOGLE_API_KEY" ? this.get("GEMINI_API_KEY") : null)
        : null

      // 获取该提供商默认模型
      const defaultModel =
        p.name === "ollama"
          ? this.get("OLLAMA_MODEL", "qwen2.5:7b")
          : p.name === "openai"
            ? this.get("OPENAI_MODEL", "gpt-4o-mini")
            : p.name === "gemini"
              ? this.get("GEMINI_MODEL", "gemini-1.5-flash")
              : p.name === "deepseek"
                ? this.get("DEEPSEEK_MODEL", "deepseek-chat")
                : p.name === "claude"
                  ? this.get("ANTHROPIC_MODEL", "claude-3-5-sonnet-20241022")
  ...
  _checkStorage() {
    ...
    return {
      type: config.type,
      available:
        fs.existsSync(config.localPath) ||
