  ...
  getLlmConfig() {
    ...
    if (this.get('OPENAI_API_KEY')) {
      providers.push({
        ...
        model: this.get('OPENAI_MODEL', 'gpt-4-turbo-preview')
      });
    }
    ...
    if (this.get('ANTHROPIC_API_KEY')) {
      providers.push({
        ...
        model: this.get('ANTHROPIC_MODEL', 'claude-3-opus-20240229')
      });
    }

    if (this.get('ZHIPU_API_KEY')) {
      providers.push({
        name: 'zhipu',
        apiKey: this.get('ZHIPU_API_KEY'),
        model: this.get('ZHIPU_MODEL', 'glm-4')
      });
    }
    ...
    return {
      providers,
      defaultProvider: this.get('LLM_DEFAULT_PROVIDER', providers[0]?.name || 'openai')
    };
  }
