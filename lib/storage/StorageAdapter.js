...
class BaseStorage {
  ...
  async getPersona(id) { throw new Error('Must implement getPersona()'); }
  async savePersona(id, data) { throw new Error('Must implement savePersona()'); }
  async deletePersona(id) { throw new Error('Must implement deletePersona()'); }
  async listPersonas() { throw new Error('Must implement listPersonas()'); }
  async getStyle(id) { throw new Error('Must implement getStyle()'); }
  async saveStyle(id, data) { throw new Error('Must implement saveStyle()'); }
  async deleteStyle(id) { throw new Error('Must implement deleteStyle()'); }
  async listStyles() { throw new Error('Must implement listStyles()'); }
  async getDraft(id) { throw new Error('Must implement getDraft()'); }
  async saveDraft(data) { throw new Error('Must implement saveDraft()'); }
  async deleteDraft(id) { throw new Error('Must implement deleteDraft()'); }
  async listDrafts(options) { throw new Error('Must implement listDrafts()'); }
  async getSettings() { throw new Error('Must implement getSettings()'); }
  async updateSettings(data) { throw new Error('Must implement updateSettings()'); }
  async getTwin(personaId) { throw new Error('Must implement getTwin()'); }
  async saveTwin(personaId, data) { throw new Error('Must implement saveTwin()'); }

  // 可选方法
  async sync() { return { success: true, message: 'Sync not implemented' }; }
  async backup() { return { success: false, message: 'Backup not implemented' }; }
  async restore(backupId) { return { success: false, message: 'Restore not implemented' }; }
}
...
class LocalStorage extends BaseStorage {
  ...
  async listBackups() {
    ...
    return fs.readdirSync(this.dirs.backups)
...
class MongoDBStorage extends BaseStorage {
  ...
  async saveStyle(id, data) {
    this._ensureInit();
    const payload = { ...data, id, updatedAt: new Date().toISOString() };
    await this.db.collection('styles').updateOne(
      { id },
      { $set: payload },
      { upsert: true }
    );
  ...
  async saveDraft(data) {
    ...
    await this.db.collection('drafts').updateOne(
      { id },
      { $set: payload },
      { upsert: true }
    );
  ...
  async listDrafts(options = {}) {
    ...
    const drafts = await this.db.collection('drafts')
      .find({})
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limit)
  ...
  async saveTwin(personaId, data) {
    this._ensureInit();
    await this.db.collection('twins').updateOne(
      { personaId },
