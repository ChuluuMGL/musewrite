...
class PreferenceLearner {
  ...
  _learnFromFeedback(record) {
    ...
    if (record.rating && record.rating < 3) {
      // 标记最近的规则为低效
      const _recentRules = this.preferences.rules
  ...
  getStats() {
    return {
      ...
      avgRating:
        this.preferences.feedback.length > 0
          ? (
            this.preferences.feedback.reduce((sum, f) => sum + (f.rating || 0), 0) /
              this.preferences.feedback.length
          ).toFixed(1)
          : null,
