...
async function main() {
  ...
  // 测试 1: 无认证
  await test('无 API Key 访问', `${BASE_URL}/api/v1/generate`, {
    method: 'POST',
  ...
  // 测试 2: 无效 API Key
  await test('无效 API Key', `${BASE_URL}/api/v1/generate`, {
    method: 'POST',
    headers: {
  ...
  await test('缺少 source 字段', `${BASE_URL}/api/v1/generate`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-API-Key': API_KEY
  ...
  // 测试 4: 无效端点
  await test('无效端点', `${BASE_URL}/api/v1/invalid`);
